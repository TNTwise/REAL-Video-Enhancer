...
def start(renderdir,videoName,videopath):
        ...
        os.system(f'ffmpeg -i "{videopath}" "{renderdir}/{videoName}_temp/input_frames/%08d.png" ') # Add image extraction setting here, also add ffmpeg command here as if its compiled or not
...
def end(renderdir,videoName,videopath,times,outputpath,videoQuality,encoder):
        ...
        os.system(f'ffmpeg -framerate {fps*times} -i "{renderdir}/{videoName}_temp/output_frames/%08d.png" -c:v libx{encoder} -crf {videoQuality} -c:a copy "{output_video_file}"') #ye we gonna have to add settings up in this bish
