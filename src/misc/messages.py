...
def show_scene_change_help(self):
    self.showDialogBox(
        "1 is the most sensitive, detecting the most frame changes in a scene.\n9 is the least sensitive, detecting fewest frame changes in a scene."
    )


def show_on_no_output_files(self):
    self.showDialogBox(
        "Output frames or Audio file does not exist. Did you accidently delete them?",
        True,
    )
...
def encoder_help(self):
    self.showDialogBox(
        ".h264 is more standardized, but has worse quality. (shorter render time) \n.h265 is less standardized, but retains more visual quality. (longer render time)"
    )


def cannot_detect_vram(self):
    self.showDialogBox(
        "Cannot detect vram amount, please set this value in settings for increased performance.\nIf you are on integrated graphics, consider leaving this value at 1.",
        True,
    )
...
def no_downloaded_models(self):
    self.showDialogBox(
        "No models selected, please select at least one model to download.", True
    )


def failed_download(self):
    self.showDialogBox(
        "Failed to download dependencies, please check your connection and try again.",
        True,
    )
...
def not_enough_vram(self):
    self.showPopup(
        "Your GPU does not meet the minimum required VRAM amount.\nMinimum: 4GB\nRecommended: 8GB\n It is highly advised to use Rife-V4 models and refrain from interpolating in resolutions past 1080p.",
        self.ignore_vram_popup,
    )


def too_large_video(self):
    self.showDialogBox(
        "WARNING: The video you have chosen exceeds the maximum resolution (3840x2160).\nThis could cause issues in the render!"
    )
...
def no_perms(self):
    self.showDialogBox(
        f"No permissions to write here, please select a different directory or the default Output directory will be {homedir}/Videos"
    )


def no_perms_render(self):
    self.showDialogBox(
        f"No permissions to write here, please select a different directory or the default Render directory will be {thisdir}"
    )
...
def no_perms_anywhere(self, dir=thisdir):
    self.showDialogBox(
        f"No permissions to write to {dir}, please select a directory in settings!!!!!"
    )
...
def alreadyModel(self):
    self.showDialogBox(
        f"Same model found in models directory, please rename it to a different model."
    )


def already_Render_folder(self):
    return self.showQuestionBox(
        f"render folder already exists within directory, this can lead to overwriting of files within this directory.\nSet directory anyway?"
    )


def ensembleModelDoesntExist(self):
    self.showDialogBox(
        "Ensemble model does not exist, please click Install/Remove models to download the ensemble version. Using the regular model instead."
    )


def ensemble_help(self):
    self.showDialogBox(
        "Ensemble interpolates a single frame twice, leading to a better result, but the render will take twice as long."
    )
...
def transition_detection_lossless_warning(self):
    self.showDialogBox(
        "Transition detection is currently unsupported with Lossless encoding."
    )


def outdated_binary(self, binary):
    self.showDialogBox(
        f"Outdated {binary} binary, click on Install/Remove in the Manage Models settings menu to download the latest binary and use newer models!"
    )


def UHDModeHelp(self):
    self.showDialogBox(
        f"UHD mode allows for flow to be calculated at a lower resolution, hopefully saving on VRAM.\nThis cutoff resolution means that anything greater than it will enable UHD mode, and lower threading on the GPU."
    )


def GPUIDHelp(self):
    self.showDialogBox(
        f"GPU ID selects what GPU to use. Usually, 0 is the best option, as ncnn generally selects the dedicated GPU.\nIf you are in a dual-gpu setup, you can set this to 1 if the render does not seem to be using the right GPU.\n-1 option means it uses CPU inference."
    )
