...
class GetLinkedWindow(QMainWindow):
    ...
    def next(self):
        ...
        self.ui.next.hide()
        if (
            "youtu.be" in self.ui.plainTextEdit.text()
            or "youtube.com" in self.ui.plainTextEdit.text()
        ):
            ...
            try:
                ...
                if content_type and content_type.startswith("video/"):
                    ...
                    self.ui.error_label.setText(
                        f"Invalid video format. Content-Type: {content_type}"
                    )
                    self.ui.next.show()
                    return False
                ...
                try:
                    ...
                    ):  # ignores 403 errors cause they mostly work even when the exception is raised :\
    ...
    def run_ytdl_thread(self):
        ...
        self.worker.finished.connect(self.end_DownloadofData)
    ...
    def report_progress(self, result):
        if (
            result
            == f"""ERROR: [generic] None: '{self.ui.plainTextEdit.text()}' is not a valid URL. Set --default-search "ytsearch" (or run  yt-dlp "ytsearch:{self.ui.plainTextEdit.text()}" ) to search YouTube\n"""
        ):
    ...
    def get_youtube_video_name(self, url):
        try:
            result = subprocess.run(
                [f"{thisdir}/bin/yt-dlp_linux", "--get-title", url],
                capture_output=True,
                text=True,
            )
            if result.returncode == 0:
