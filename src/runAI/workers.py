        ...
        try:
            if "-ncnn-vulkan" in self.main.AI:
                while (
                    ...
                    sleep(
                        0.1
                    )  # has to refresh quickly or small files that interpolate fast do not work

                total_input_files = len(
                    os.listdir(
                        f"{self.settings.RenderDir}/{self.videoName}_temp/input_frames/"
                    )
                )
                ...
                while (
                    ManageFiles.isfolder(
                        f"{self.settings.RenderDir}/{self.videoName}_temp/"
                    )
                    == True
                ):
                    if (
                        ...
                        try:
                            if self.settings.RenderType == "Optimized (Incremental)":
                                try:
                                    files_processed = os.listdir(
                                        f"{self.settings.RenderDir}/{self.videoName}_temp/output_frames/0/"
                                    )
                                    files_processed.sort()
                                    files_processed = files_processed[-1]
                                    files_processed = files_processed.replace(
                                        self.settings.Image_Type, ""
                                    )
                                    files_processed = int(files_processed)
                                    self.main.files_processed = int(
                                        (
                                            len(
                                                os.listdir(
                                                    f"{self.settings.RenderDir}/{self.videoName}_temp/output_frames/"
                            ...
                            if self.main.imageDisplay != None:
                                try:
                                    if os.path.exists(self.main.imageDisplay):
                                        ...
                                        self.main.height1 = int(
                                            self.main.width1 / self.main.aspectratio
                                        )
                                        if self.main.height1 >= height / 1.6:
                                            self.main.height1 = int(height / 1.6)
                                            self.main.width1 = int(
                                                self.main.height1
                                                / (
                                                    self.main.videoheight
                                                    / self.main.videowidth
                                                )
                                            )
                                        try:
                                            if os.path.exists(self.main.imageDisplay):
                    ...
                    while self.main.CudaRenderFinished == False:
                        try:
                            ...
                            if self.main.height1 >= height / 1.6:
                                ...
                                self.main.width1 = int(
                                    self.main.height1
                                    / (self.main.videoheight / self.main.videowidth)
                                )
                            self.image_progress.emit("2")
                        except Exception as e:
                            traceback_info = traceback.format_exc()
    ...
    def run(self):
        ...
        try:
            result = subprocess.run(
                [f"{thisdir}/bin/yt-dlp_linux", "-F", self.url],
                capture_output=True,
                text=True,
            )
            ...
            if result.returncode == 0:
                ...
                self.originalSelf.duration = (
                    self.originalSelf.get_youtube_video_duration(self.url)
                )
...
def calculateVRAM(self):
    ...
    if (
        int(height) > int(self.main.settings.UHDResCutOff)
    ):  # check if resolution is greater than cutoff, if it is set to 2 if threading val is greater than or equal to 4
        ...
        while True:
            try:
                if (
                    ...
                    or iteration == interpolation_sessions - 1
                ):
                    j = 1

                    if iteration == interpolation_sessions - 1:
                        total_frames_rendered = abs(
                            (interpolation_sessions - 1)
                            * self.main.frame_increments_of_interpolation
                            - self.main.frame_count
                        )
                    ...
                    for i in range(
                        self.main.frame_increments_of_interpolation
                    ):  # removes previous frames, takes the most time (optimize this?)
                        os.system(
                            f'rm -rf "{self.main.settings.RenderDir}/{self.main.videoName}_temp/output_frames/0/{str(i+((iteration-1)*self.main.frame_increments_of_interpolation)).zfill(8)}{self.main.settings.Image_Type}"'
                        )
...
class interpolation(QObject):
    ...
    def Render(self, model, times, videopath, outputpath):
        ...
        if "-ncnn-vulkan" in self.main.AI:
            ...
            self.main.frame_count = (
                self.input_frames * self.main.times
            ...
            if self.main.AI == "rife-ncnn-vulkan":
                ...
                command = [
                    ...
                    "-j",
                    f"{math.ceil(vram/4)}:{vram}:{math.ceil(vram/4)+1}",
                    "-f",
                    f"%08d{self.main.settings.Image_Type}",
                    "-g",
            ...
            if self.main.AI == "ifrnet-ncnn-vulkan":
                command = [
                    ...
                    f"%08d{self.main.settings.Image_Type}",
                    "-m",
                    f"{settings.ModelDir}ifrnet/{self.main.ui.Rife_Model.currentText()}",
                    "-g",
                    f"{self.main.ui.gpuIDSpinBox.value()}",
                ]
        ...
        if "cuda" in self.main.AI or "ncnn-python" in self.main.AI:
            ...
            if self.main.AI == "realcugan-ncnn-python":
                ...
                self.main.renderAI = RenderCUDA.Upscaling(
                    ...
                    output_file,
                    int(self.main.ui.Rife_Times.currentText()[0]),
                    half=bool(self.main.ui.halfPrecisionCheckBox.isChecked()),
                    method=self.main.AI,
                    threads=int(settings.VRAM),
                    ncnn_gpu=self.main.ui.gpuIDSpinBox.value(),
