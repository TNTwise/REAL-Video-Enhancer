...
class FFMpegInfoWrapper(VideoInfo):
    ...
    def _get_ffmpeg_info(self):
        ...
        for line in self.ffmpeg_output_raw.split('\n'):
            if 'Stream #' in line and 'Video' in line:
                ...
                self.ffmpeg_output_raw = self.ffmpeg_output_raw.replace(
                    line, ''
                )
                break
    ...
    def get_duration_seconds(self) -> float:
        ...
        duration = re.search(
            r'duration: (.*?),', self.ffmpeg_output_stripped
        ).groups()[0]
    ...
    def get_fps(self) -> float:
        fps = re.search(
            r'(\d+\.?\d*) fps', self.ffmpeg_output_stripped
        ).groups()[0]
        return float(fps)
    ...
    def get_pixel_format(self) -> str:
        if self.stream_line:
            try:
                pixel_format = (
                    self.stream_line.split(',')[1].split('(')[0].strip()
                )
                return pixel_format
            except Exception:
...
class OpenCVInfo(VideoInfo):
    def __init__(
        ...
        self.ffmpeg_info = FFMpegInfoWrapper(
            input_file, ffmpeg_path=ffmpeg_path
        )
