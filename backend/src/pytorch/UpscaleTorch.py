...
class UpscalePytorch:
    ...
    def _load(self):
        ...
        with self.torchUtils.run_stream(self.prepareStream):
            ...
            if all(t > 0 for t in self.tile):
                self.pad_w = (
                    math.ceil(
                        min(self.tile[0] + 2 * self.tile_pad, self.videoWidth)
                        / modulo
                    )
                    * modulo
                )
                self.pad_h = (
                    math.ceil(
                        min(self.tile[1] + 2 * self.tile_pad, self.videoHeight)
                        / modulo
                    )
                    * modulo
                )
                ...
                modulo = (
                    1 if self.videoWidth < 720 or self.videoHeight < 720 else 1
                )
            ...
            if self.backend == 'tensorrt':
                self.tensorrt_example_inputs = (
                    self.upscale_model_wrapper.get_dummy_input(
    ...
    def renderTiledImage(
        ...
        output = img.new_zeros(output_shape).to(
