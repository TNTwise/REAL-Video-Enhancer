...
class ATDArch(Architecture[ATD]):
    ...
    def load(self, state_dict: StateDict) -> ImageModelDescriptor[ATD]:
        ...
        window_size = math.isqrt(
            state_dict['relative_position_index_SA'].shape[0]
        )
        ...
        for i in range(num_layers):
            depths[i] = get_seq_len(
                state_dict, f'layers.{i}.residual_group.layers'
            )
        ...
        resi_connection = (
            '1conv' if 'layers.0.conv.weight' in state_dict else '3conv'
        )
