...
class UformerArch(Architecture[Uformer]):
    ...
    def load(self, state_dict: StateDict) -> ImageModelDescriptor[Uformer]:
        ...
        if (
            'encoderlayer_0.blocks.0.attn.qkv.to_q.depthwise.weight'
            in state_dict
        ):
            ...
            token_projection = 'linear'
            qkv_bias = (
                'encoderlayer_0.blocks.0.attn.qkv.to_q.bias' in state_dict
            )

        modulator = 'decoderlayer_0.blocks.0.modulator.weight' in state_dict
        cross_modulator = (
            'decoderlayer_0.blocks.0.cross_modulator.weight' in state_dict
        )
        ...
        if 'encoderlayer_0.blocks.0.mlp.fc1.weight' in state_dict:
            ...
            state_dict['encoderlayer_0.blocks.0.mlp.dwconv.0.weight'].shape[1]
            == 1
        ):
            token_mlp = 'leff'
            mlp_ratio = (
                state_dict[
                    'encoderlayer_0.blocks.0.mlp.linear1.0.weight'
                ].shape[0]
                / embed_dim
            )
        else:
