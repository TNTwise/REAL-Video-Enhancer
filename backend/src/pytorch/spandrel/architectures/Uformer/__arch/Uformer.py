...
class SepConv2d(torch.nn.Module):
    def __init__(
        ...
        self.act_layer = act_layer() if act_layer is not None else nn.Identity()  # type: ignore
        self.in_channels = in_channels
        self.out_channels = out_channels
...
class ConvProjection(nn.Module):
    ...
    def forward(self, x, attn_kv=None):
        _b, n, _c, h = *x.shape, self.heads
        l = int(math.sqrt(n))  # noqa: E741
        w = int(math.sqrt(n))
...
class LinearProjection(nn.Module):
    ...
    def flops(self, q_L, kv_L=None):
        kv_L = kv_L or q_L
        flops = (
            q_L * self.dim * self.inner_dim
            + kv_L * self.dim * self.inner_dim * 2
        )
        return flops
...
def window_reverse(windows, win_size, H, W, dilation_rate=1):
    ...
    if dilation_rate != 1:
        x = windows.permute(
            0, 5, 3, 4, 1, 2
        ).contiguous()  # B, C*Wh*Ww, H/Wh*W/Ww
        x = F.fold(
            x,
            (H, W),
            kernel_size=win_size,
...
class OutputProj(nn.Module):
    def __init__(
        ...
        if act_layer is not None:
            self.proj.add_module(act_layer(inplace=True), None)
        if norm_layer is not None:
            self.norm = norm_layer(out_channel)
        else:
            self.norm = None
...
class LeWinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        attn_windows = self.attn(
            wmsa_in, mask=attn_mask
        )  # nW*B, win_size*win_size, C

        # merge windows
        attn_windows = attn_windows.view(-1, self.win_size, self.win_size, C)
        shifted_x = window_reverse(
            attn_windows, self.win_size, H, W
        )  # B H' W' C
