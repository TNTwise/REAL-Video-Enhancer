...
class AnchorLinear(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, down_factor, pooling_mode, bias
    ):
        super().__init__()
    ...
    def forward(self, x, x_size):
        ...
        x = blc_to_bhwc(
            self.reduction(x), [s // self.down_factor for s in x_size]
        )  # type: ignore
        return x
    ...
    def __init__(
        ...
        if one_stage:
            if proj_type == 'patchmerging':
                ...
                stride = anchor_window_down_factor
                m = SeparableConv(
                    dim, dim // 2, kernel_size, stride, True, args
                )
                ...
                raise ValueError(
                    f'Unsupported anchor projection type {proj_type}'
                )
            self.body.append(m)
        else:
