    ...
    def load(self, state_dict: StateDict) -> ImageModelDescriptor[_PLKSR]:
        ...
        if 'feats.1.channe_mixer.0.weight' in state_dict:
            ...
            if 'feats.1.lk.conv.weight' in state_dict:
                ...
                split_ratio = (
                    state_dict['feats.1.lk.conv.weight'].shape[0] / dim
                )
            elif 'feats.1.lk.convs.0.weight' in state_dict:
                # SparsePLKConv2d
                lk_type = 'SparsePLK'
                split_ratio = (
                    state_dict['feats.1.lk.convs.0.weight'].shape[0] / dim
                )
                ...
                split_ratio = (
                    state_dict['feats.1.lk.mn_conv.weight'].shape[0] / dim
                )
            else:
...
__all__ = ['PLKSR', 'PLKSRArch', 'RealPLKSR']
