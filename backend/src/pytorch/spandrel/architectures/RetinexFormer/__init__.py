...
class RetinexFormerArch(Architecture[RetinexFormer]):
    ...
    def load(
    ...
    ) -> ImageModelDescriptor[RetinexFormer]:
        in_channels = 3
        out_channels = 3
        n_feat = 40
        stage = 3
        ...
        num_blocks = [
            get_seq_len(
                state_dict, 'body.0.denoiser.encoder_layers.0.0.blocks'
            ),
            get_seq_len(
                state_dict, 'body.0.denoiser.encoder_layers.1.0.blocks'
            ),
...
__all__ = ['RetinexFormer', 'RetinexFormerArch']
