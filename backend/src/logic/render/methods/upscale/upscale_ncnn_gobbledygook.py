...
def get_ncnn_scale(modelPath: str = "") -> int:
    basename = os.path.basename(modelPath)
    try:
        modelParamPath = os.path.join(modelPath, basename + ".param")
        modelBinPath = os.path.join(modelPath, basename + ".bin")
        model = NcnnModel.load_from_file(modelParamPath, modelBinPath)
        scale = get_broadcast_data(model)[0]
        if not isinstance(scale, int):
            raise Exception(
                "getting ncnn scale failed, I dont understand this code, so good luck."
            )
    except Exception:
