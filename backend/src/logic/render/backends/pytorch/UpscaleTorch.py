...
class UpscalePytorch:
    ...
    def _load(self):
        ...
        if self.videoWidth > 1920 or (
            self.videoHeight > 1920 and not self.trt_static_shape
        ):
            logger.warning(
        ...
        if self.videoWidth < 128 or (
            self.videoHeight < 128 and not self.trt_static_shape
        ):
            logger.warning(
