        ...
        if self.backend == "tensorrt":
            ...
            if not trtHandler.check_engine_exists(base_trt_engine_name):
                if self.trt_static_shape:
                    ...
                    if self.encode is not None:
                        flownet_inputs = (
                            ...
                            torch.zeros(
                                [1, num_ch_for_encode] + self.trt_opt_shape,
                                dtype=self.dtype,
                                device=self.device,
                            ),
                            torch.zeros(
                ...
                flownet_engine = trtHandler.build_engine(
                    ...
                    self.device,
                    flownet_inputs,
                    trt_engine_name=base_trt_engine_name,
                    trt_multi_precision_engine=True,
                    dynamic_shapes=flownet_dynamic_shapes,
                )
                ...
                if self.encode:
                    encode_engine = trtHandler.build_engine(
                        self.encode,
                        self.dtype,
                        self.device,
                        encode_inputs,
                        trt_engine_name=encode_trt_engine_name,
                        trt_multi_precision_engine=True,
                        dynamic_shapes=encode_dynamic_shapes,
                    )
                    trtHandler.save_engine(
                        encode_engine, encode_trt_engine_name, encode_inputs
                    )
                TorchUtils.clear_cache()
        ...
        with self.torchUtils.run_stream(self.stream):  # type: ignore
            with self.torchUtils.run_stream(self.prepareStream):  # type: ignore
                if self.frame0 is None:
                    ...
                    if self.encode:
                        self.encode0 = self.encode_Frame(
                            self.frame0, self.prepareStream
                        )
                    return
