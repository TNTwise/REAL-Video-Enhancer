...
torch_tensorrt = (
    ...
    if importlib.util.find_spec("torch_tensorrt")
    else None
)
...
def _build_inputs(device, dtype=torch.float16, height=1088, width=1920, seed=1234):
    ...
    img0 = torch.rand(
        1, 3, height, width, device=device, dtype=dtype, generator=generator
    )
    img1 = torch.rand(
        1, 3, height, width, device=device, dtype=dtype, generator=generator
    )
    tenFlow = torch.rand(
        1, 2, height, width, device=device, dtype=dtype, generator=generator
    )
    tenFlow_div = torch.tensor([1.0, 1.0], device=device, dtype=dtype)
    backwarp_tenGrid = torch.rand(
        1, 2, height, width, device=device, dtype=dtype, generator=generator
    )
...
def save_output_image(output: torch.Tensor, path: str):
    ...
    image_array = (image_tensor[:3].permute(1, 2, 0) * 255.0).to(torch.uint8).numpy()
