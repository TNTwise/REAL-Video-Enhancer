...
from src.utils.LogConfig import configure_logging, get_logger
...
class HandleApplication:
    def __init__(self):
        ...
        if not self.args.list_backends:
            ...
            if self.args.print_video_info:
                ...
                video_info = OpenCVInfo(
                    self.args.input, ffmpeg_path=self.ffmpeg_path
                )
                print_video_info(video_info)
    ...
    def batchProcessing(self) -> bool:
        ...
        if os.path.splitext(self.args.input)[-1] == '.txt':
            with pathlib.Path(self.args.input).open() as f:
                for line in f:  # iterate through each render
                    sys.argv[1:] = (
                        line.split()
    ...
    def listBackends(self):
        ...
        if ncnn_ver:
            ...
            printMSG += 'NCNN Version: 20220729\n'
    ...
    def checkArguments(self):
        if (
            ...
            and pathlib.Path(self.args.output).is_file()
        ...
        if 'http' not in self.args.input:
            if not pathlib.Path(self.args.input).is_file():
