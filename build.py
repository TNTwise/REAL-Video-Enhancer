...
def get_libxcb_cursor_binary():
    try:
        ...
        try:
            ...
            if CPU_ARCH == "x86_64":
                download_file(
                    "https://github.com/TNTwise/real-video-enhancer-models/releases/download/models/libxcb-cursor.so.0",
                    "libxcb-cursor.so.0",
                )
            else:
                download_file(
                    "https://github.com/TNTwise/real-video-enhancer-models/releases/download/models/libxcb-cursor.so.0_arm64",
                    "libxcb-cursor.so.0",
                )
            ...
            raise FileNotFoundError(
                "libxcbcursor not installed, and no network available to download it!"
            )
    return input_file
...
class PythonManager:
    PYTHON_VENV_PATH = (
        "venv\\Scripts\\python.exe" if PLATFORM == "win32" else "venv/bin/python3"
    )
...
class PyInstaller(BuildManager):
    ...
    def patch_for_xcbcursor(self):
        if PLATFORM == "linux":
            ...
            shutil.copy(
                input_file,
                f"{OUTPUT_FOLDER}/REAL-Video-Enhancer/_internal/PySide6/Qt/lib/",
            )
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--build",
        help="Build the application with a specific builder.",
        default="gui",
        choices=["pyinstaller", "cx_freeze", "nuitka", "gui"],
    )
    args.add_argument(
        "--copy_backend",
        help="Copy the backend to the build directory",
        action="store_true",
    )
