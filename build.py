...
def check_signature_rewritable(graph):
    ...
    for node in graph.graph.find_nodes(op="placeholder"):
        ...
        if stack is None:
            ...
            msg = f"{source.name()}, accessed at:{tb}{extra}"
...
def fixEvalFrame():
    with open(os.path.join("dist","main","_internal","torch","_dynamo","eval_frame.py") , 'w') as f:
...
def buildNCNNMacOS():
    ...
    '--onefile', 'main.py',
    '--hidden-import', 'upscale_ncnn_py.upscale_ncnn_py_wrapper',
    '--hidden-import', 'realcugan_ncnn_py.realcugan_ncnn_py_wrapper',
